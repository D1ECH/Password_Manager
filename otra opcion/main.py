...
def register(username, master_password):
    # Cifrado de la contraseña maestra antes de almacenarla
    hashed_master_password = hash_password(master_password)
    
    # Creación de la base de datos y tabla si no existen
    ...
    try:
        cursor.execute("INSERT INTO secrets (username, master_password) VALUES (?, ?)",
                       (username, hashed_master_password))
        conn.commit()
...
if __name__ == "__main__":
    if os.path.isfile(DB_PATH) and os.path.getsize() != 0:
        try:
            # Intentar conectarse para comprobar si es un archivo SQLite válido
            conn = sql.connect(DB_PATH)
            cursor = conn.cursor()
            ...
            if tablas:
                print(f"La base de datos en {ruta_bd} es válida y contiene tablas.")
            else:
                print(f"La base de datos en {ruta_bd} es válida pero no contiene tablas.")

            conn.close()
        except sql.DatabaseError as e:
            print(f"El archivo en {ruta_bd} no es una base de datos SQLite válida. Error: {e}")
    else:
        conn = sql.connect(DB_PATH)
        cursor = conn.cursor()
